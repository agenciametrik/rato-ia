...
# Agência Metrik · RatosOS

Neste arquivo, "você" é o dono do sistema; as instruções são pro agente.

## 1. O que é este sistema

Workspace de operações da Agência Metrik, uma agência de marketing digital e performance com
base em Teresina-PI e atuação nacional com negócios locais. Atendemos empresas locais que
precisam gerar demanda, leads e vendas: educação superior, restaurantes/delivery, automotivo,
odontologia, jurídico, energia solar, imobiliário, entre outros. Serviços principais: tráfego
pago (Meta Ads, Google Ads), criativos e social media, CRM e automação, tracking (Meta Pixel,
GA4, GTM), análise de vendas cruzando mídia com faturamento real. Equipe: Álvaro (comercial e
gestão de projetos), Guilherme (operação), Júnior (tráfego e social media) e Edilson (design e
financeiro). O que mais produzimos aqui: propostas comerciais, relatórios de performance,
diagnósticos, criativos e conteúdo próprio da agência.
